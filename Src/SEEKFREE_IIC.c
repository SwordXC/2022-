...
#define SDA   			MPU6050_SDA_GPIO_Port->IDR & MPU6050_SDA_Pin
#define SDA0()          HAL_GPIO_WritePin (MPU6050_SDA_GPIO_Port,MPU6050_SDA_Pin,(GPIO_PinState)0)		//IO口输出低电平
#define SDA1()          HAL_GPIO_WritePin (MPU6050_SDA_GPIO_Port,MPU6050_SDA_Pin,(GPIO_PinState)1)		//IO口输出高电平  
#define SCL0()          HAL_GPIO_WritePin (MPU6050_SCL_GPIO_Port,MPU6050_SCL_Pin,(GPIO_PinState)0)		//IO口输出低电平
#define SCL1()          HAL_GPIO_WritePin (MPU6050_SCL_GPIO_Port,MPU6050_SCL_Pin,(GPIO_PinState)1)		//IO口输出高电平
#define DIR_OUT()		;//不需要
#define DIR_IN()		;//不需要
...
void simiic_delay(void)
{
    ...
    uint16_t j=100;   //使用小钻风摄像头时，此延时需要设置为500左右
	while(j--);
}
